...
```
ngram/
├── ngram_proposer.py          # 核心实现：KMP / Hash / Trie / SkipGram / PySuffix（symlink 到 vllm）
├── py_suffix_tree.py          # Python 压缩后缀树（100% 复现 C++ SuffixTree）
...
├── docs/
│   ├── 现有方案分析.md         # 各方案原理深入分析
│   ├── suffix方案分析.md       # Suffix Decoding 深度分析（C++ 源码级）
...
│   ├── test_pysuffix_vs_cpp.py # PySuffix vs C++ Suffix benchmark
│   ├── test_pysuffix_consistency.py # PySuffix 一致性单元测试
│   ├── suffix_decoding.py     # vLLM SuffixDecodingProposer 本地副本
│   ├── ...                    # 其他历史测试脚本
...
### 五、PySuffix — Python 复现 C++ SuffixTree（100% 一致）

纯 Python 实现的压缩后缀树（`py_suffix_tree.py`），完整复现 arctic-inference C++ `SuffixTree`：

- **数据结构**：路径压缩 + count 排序兄弟链表 + Group 分组机制
- **浮点精度**：`struct.pack/unpack` 匹配 C++ float32
- **集成**：通过 `input_batch` 获取 `num_prompt_tokens`、`req_ids`，与 C++ 调用路径完全对齐

| Mode | Drafts | DraftTok | Accept% | MeanLen | Speedup |
|---|---|---|---|---|---|
| C++ Suffix | 3480 | 9277 | 53.2% | 2.42 | 1.84x |
| PySuffix | 3480 | 9277 | 53.2% | 2.42 | 1.84x |

**所有指标完全一致**（包括 per-position acceptance rate）。详见 `docs/suffix方案分析.md`。
...
| 03-13 | PySuffix vs C++ Suffix | 旧版 per-root-key trie，Python overhead 较大 |
| 03-22 | PySuffix 重写为压缩后缀树 | 100% 输出一致（Drafts/DraftTok/Accept% 全同） |
