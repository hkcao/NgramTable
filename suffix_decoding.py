...
class SuffixDecodingProposer:
    ...
    def __init__(self, vllm_config: VllmConfig):
        ...
        # Minimum match length filter (n-gram root analogy).
        # Drafts from matches shorter than this are discarded.
        self.min_match_len = int(os.environ.get(
            "VLLM_SUFFIX_MIN_MATCH_LEN", "0"))
        if self.min_match_len > 0:
            logger.info("Suffix min_match_len=%d (filtering short matches)",
                        self.min_match_len)
        ...
        self._debug_fp = None
        self._debug_step = 0
        if os.environ.get("VLLM_SUFFIX_DEBUG") == "1":
            self._debug_fp = open("/tmp/suffix_debug.log", "w")
            logger.info("Suffix debug log: /tmp/suffix_debug.log")
    ...
    def propose(
        ...
        for i, sampled_ids in enumerate(sampled_token_ids):
            ...
            # Debug logging
            if self._debug_fp is not None and draft.token_ids:
                self._debug_step += 1
                ctx_tail = pattern[-3:].tolist() if len(pattern) >= 3 \
                    else pattern.tolist()
                self._debug_fp.write(
                    f"step={self._debug_step} req={req_id} "
                    f"ctx_tail={ctx_tail} match_len={draft.match_len} "
                    f"draft={draft.token_ids}\n")
                self._debug_fp.flush()

            # Filter out drafts from short matches (n-gram root analogy).
