"""
Suffix Decoding: effect of min_match_len filter (n-gram root analogy).
Tests match_len thresholds 0 (original), 2, 3, 4, 5.
"""
import argparse
import gc
import json
...
def _run(prompt_texts, model_name, gpu_mem, max_tokens,
    ...
    os.environ["VLLM_SUFFIX_MIN_MATCH_LEN"] = str(min_match_len)
    # Ensure we're NOT using ngram modes
    os.environ["VLLM_NGRAM_USE_HASH"] = "0"
    os.environ["VLLM_NGRAM_USE_TRIE"] = "0"
    ...
    try:
        print(f"\n{'='*70}")
        print(f"Mode: {mode_name}  (min_match_len={min_match_len})")
        print(f"{'='*70}")
...
def run_subprocess(prompt_texts, model_name, gpu_mem, max_tokens,
    ...
    p = ctx.Process(
        ...
        args=(prompt_texts, model_name, gpu_mem, max_tokens,
              spec_config, mode_name, num_warmup, q, min_match_len),
    )
    p.start()
    p.join(timeout=1200)
    if p.is_alive():
...
def main():
    parser = argparse.ArgumentParser(
        description="Suffix min_match_len benchmark")
    ...
    configs = [
        ("baseline", None, 0),
        ("Suffix (original)", suffix_spec, 0),
        ("Suffix min=2", suffix_spec, 2),
        ("Suffix min=3", suffix_spec, 3),
        ("Suffix min=4", suffix_spec, 4),
        ("Suffix min=5", suffix_spec, 5),
    ]

    for i, (name, sc, mml) in enumerate(configs):
    ...
    # Summary
    print("\n\n" + "=" * 120)
    print("SUFFIX MIN_MATCH_LEN COMPARISON")
    print("=" * 120)
    ...
    hdr = (f"{'Mode':<25} {'AvgLat':>8} {'MedLat':>8} {'P90Lat':>8} "
           ...
           f"{'Drafts':>8} {'DraftTok':>8} {'AccTok':>8}")
    print(hdr)
    print("-" * 130)
