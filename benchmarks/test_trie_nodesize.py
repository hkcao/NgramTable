"""
Quick benchmark: Trie node_size=1 vs 2 vs 3 (+ baseline).

Only tests spec=5 config to save time.
"""
import argparse
import gc
import json
...
def _run(prompt_texts, model_name, gpu_mem, max_tokens,
         ...
         trie_node_size=1, trie_internal_node_size=1):
    import torch as _torch

    os.environ["VLLM_NGRAM_USE_HASH"] = "0"
    os.environ["VLLM_NGRAM_USE_TRIE"] = "1" if spec_config else "0"
    os.environ["VLLM_TRIE_NODE_SIZE"] = str(trie_node_size)
    os.environ["VLLM_TRIE_INTERNAL_NODE_SIZE"] = str(trie_internal_node_size)
    ...
    try:
        print(f"\n{'='*70}")
        print(f"Mode: {mode_name}  (root_ns={trie_node_size}, "
              f"internal_ns={trie_internal_node_size})")
        print(f"{'='*70}")
...
def run_subprocess(prompt_texts, model_name, gpu_mem, max_tokens,
    ...
    p = ctx.Process(
        ...
        args=(prompt_texts, model_name, gpu_mem, max_tokens,
              ...
              trie_node_size, trie_internal_node_size),
    )
    p.start()
    p.join(timeout=1200)
    if p.is_alive():
...
def main():
    parser = argparse.ArgumentParser(
        description="Trie node_size benchmark")
    ...
    # (name, spec_config, root_node_size, internal_node_size)
    configs = [
        ("baseline", None, 1, 1),
        ("Trie-1g (original)", spec_config, 1, 1),
        ("Trie-2g root-only", spec_config, 2, 1),
        ("Trie-3g root-only", spec_config, 3, 1),
        ("Trie-2g full", spec_config, 2, 2),
        ("Trie-3g full", spec_config, 3, 3),
        ("Trie-2g root+3g int", spec_config, 2, 3),
    ]

    for i, (name, sc, ns, ins) in enumerate(configs):
        print(f"\n>>> [{i+1}/{len(configs)}] {name} ...")
        r = run_subprocess(
            prompt_texts, args.model, args.gpu_mem, args.max_tokens,
            spec_config=sc, mode_name=name,
            trie_node_size=ns,
            trie_internal_node_size=ins,
        )
    ...
    # Summary
    print("\n\n" + "=" * 130)
    print("TRIE NODE SIZE COMPARISON")
    print("=" * 130)
    ...
    hdr = (f"{'Mode':<25} {'AvgLat':>8} {'MedLat':>8} {'P90Lat':>8} "
    ...
    for r in valid:
        ...
        print(f"{r['mode']:<25} {r['avg_latency']:>8.3f} "
