"""
Single-request sequential benchmark: HashTable vs KMP vs Baseline.

Sends one prompt at a time to measure per-request latency,
which is the primary use case for speculative decoding.
"""
...
# ---------------------------------------------------------------------------
# Data preparation (reuse from test_hash_vs_kmp.py)
# ---------------------------------------------------------------------------
...
class SingleSeqResult:
    ...
    per_request: list  # list of {input_tokens, output_tokens, latency, tps}
...
def run_benchmark(model_name, num_samples, max_tokens, ngram_configs,
    ...
    print("=" * 70)
    print("Single-Request Sequential Benchmark")
    ...
    # 2. KMP then HashTable for each config
    ...
    # 3. Summary
    print("\n\n" + "=" * 120)
    print("SINGLE-REQUEST SEQUENTIAL RESULTS")
    print("=" * 120)
    ...
    for r in valid:
        speedup = base_tps / r["output_tokens_per_second"] \
            if r["output_tokens_per_second"] > 0 else 0
    ...
    # Pairwise
    print("\n" + "=" * 70)
    print("KMP vs HashTable Pairwise (Latency)")
    print("=" * 70)
    ...
    for kmp_name, kmp_r in kmp_results.items():
        ...
        print(f"    HashTable: avg_lat={ht_avg:.3f}s  "
              f"tps={ht_tps:.1f}  accept={ht_acc:.1f}%")
        print(f"    HT/KMP:   latency {lat_ratio:.3f}x  "
              f"throughput {tps_ratio:.3f}x")

    # Save
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            "single_seq_results.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({
            "model": model_name,
            "num_samples": num_samples,
            "max_tokens": max_tokens,
            "mode": "single_request_sequential",
            "results": all_results,
...
def main():
    parser = argparse.ArgumentParser(
        description="Single-request sequential benchmark")
